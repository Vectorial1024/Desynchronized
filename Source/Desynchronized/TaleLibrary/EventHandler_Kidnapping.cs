...
namespace Desynchronized.TaleLibrary
{
    public class EventHandler_Kidnapping
    {
        ...
        {
            bool hasEverLoopedPastVictim = false;
            foreach (Pawn other in PawnsFinder.AllMapsCaravansAndTravelingTransportPods_Alive_OfPlayerFaction)
            {
                ...
                if (!hasEverLoopedPastVictim && other == victim)
                {
                    hasEverLoopedPastVictim = true;
                    continue;
                }
                ...
                TaleNewsColonistKidnapped news = new TaleNewsColonistKidnapped(other, victim, kidnapper);
